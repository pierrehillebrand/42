...
static void	check_number(t_game *game, int i, int j, int c)
{
	...
	if (c == 0 || e != 1 || p != 1)
		clean_exit(game, "The number of special elements is incorrect\n");
	game->collectibles = c;
	if (!reach_check(game))
		clean_exit(game, "This map is not beatable\n");
}
...
static int	test_files(char *path, t_game *game)
{
	...
	if (ft_strlen(path) < 4)
		clean_exit(game, "File is not valid\n");
	if (!(path[ft_strlen(path) - 1] == 'r' && path[ft_strlen(path) - 2] == 'e'))
		clean_exit(game, "Map is not .ber\n");
